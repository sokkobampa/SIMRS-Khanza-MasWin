...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            skp_kategori_penilaian=false,skp_kriteria_penilaian=false,skp_penilaian=false,referensi_poli_mobilejknfktp=false,referensi_dokter_mobilejknfktp=false,skp_rekapitulasi_penilaian=false;
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.skp_penilaian,user.referensi_poli_mobilejknfktp,user.referensi_dokter_mobilejknfktp,user.skp_rekapitulasi_penilaian from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
