...
<html>
    ...
    <?php
        ...
        if((USERHYBRIDWEB==$usere)&&(PASHYBRIDWEB==$passwordte)){
            ...
            $tglAwal    = getOne("select date_format(reg_periksa.tgl_registrasi, '%e %M %Y') from reg_periksa where reg_periksa.no_rawat = '$norawat'");
            $jamAwal    = getOne("select reg_periksa.jam_reg from reg_periksa where reg_periksa.no_rawat = '$norawat'");
            $tglAkhir   = getOne("select date_format(kamar_inap.tgl_keluar, '%e %M %Y') from kamar_inap where kamar_inap.no_rawat = '$norawat' and kamar_inap.stts_pulang != 'Pindah Kamar' order by tgl_keluar desc, jam_keluar desc");
